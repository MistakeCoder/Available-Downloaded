...
@Component({
  ...
  providers: [MyProfileService, DatePipe]
})
export class OrderComponent extends TablePage implements OnInit {
  ...
  getItems() {
    ...
    this.myProfileService.getAllOrder(params, this.page.current, this.page.perPage).subscribe((res: any) => {
      this.setData(res);
 ...
 */
  ...
  public onColumnClick(event: any): void {
    ...
    this.router.navigate(['/payment'], { queryParams: { idOrder: event.row, type: 'order' }, });
  }
}
